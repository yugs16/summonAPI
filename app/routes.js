  ...
  api.post('/signin', function (req, res) {
    ...
    User.findOne({
      ...
      if (!user) {
        ...
        var passwordToEncrypt = req.body.password + passwordKey;
        bcrypt.compare(passwordToEncrypt, user.password, function (err, hash) {
          console.log(hash);
          if (!res) {
  ...
  api.post('/signup', function (req, res) {
    ...
    User.findOne({
      ...
      if (user) {
        ...
        var passwordToEncrypt = req.body.password + passwordKey;
        bcrypt.hash(passwordToEncrypt, null, null, function (err, hash) {
        ...
        if (user) {
          ...
          post.save(function (err, instance) {
            ...
            if (instance) {
              ...
              user.update({ 'post_cnt': cnt, 'contributions_cnt': increaseCnt, 'contributions_points': points, $push: { 'posts': instance._id } }, function (err, user) {
                console.log("updated");
                res.status(200).json({
                  status: 200,
                  status_info: "success",
